...
function searchDrinks() {
    let drinks = document.getElementById('search-bebidas').value.toLowerCase();

    console.log("Buscando con los siguientes criterios:");
    console.log("Drinks:", drinks);

    let filteredDrinks = drinks.filter(drink => {
        let drinksMatch = drinks === '' || drink.name.toLowerCase().includes(drinks);

        console.log(`Drink: ${drink.name} - Drinks Match: ${drinksMatch}`);
...
function updateTable(listDrinks  = drinks) {
    let cuerpo = "";
    let category;
    let status;

    listDrinks.forEach(function (elemento) {
        ...
        let registro = '<tr>' +
            ...
            '<td>' + Number(listDrinks.indexOf(elemento) + 1) + '</td>' +
            '<td>' + elemento.name + '</td>' +
...
function searchDrinks() {
    let nameDrink = document.getElementById('search-drinks').value.toLowerCase();
    let filteredDrink = drinks.filter(drink => {
        let nameMatch = drink.name.toLowerCase().includes(nameDrink);
        return nameMatch;
    });
    loadBranch(filteredDrink);
}

function loadBranch(filteredDrink) {
    updateTable(filteredDrink)
}
