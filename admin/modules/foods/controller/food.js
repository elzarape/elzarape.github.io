...
function updateTable(listFoods = foods) {
    let cuerpo = "";
    let category;
    let status;

    listFoods.forEach(function (elemento) {
        ...
        let registro = '<tr>' +
            ...
            '<td>' + Number(listFoods.indexOf(elemento) + 1) + '</td>' +
            '<td>' + elemento.name + '</td>' +
...
function searchFood() {
    let nameFood = document.getElementById('search-food').value.toLowerCase();
    let filteredFood = foods.filter(food => {
        let nameMatch = food.name.toLowerCase().includes(nameFood);
        return nameMatch;
    });
    loadBranch(filteredFood);
}

function loadBranch(filteredFood) {
    updateTable(filteredFood)
}
